...
class RetroDisplay:
    ...
    def _refresh(self):
        ...
        progress_lines = (len(self.progress_line) // 100) + 1 if self.progress_line else 0
...
def transcribe_segment(audio_path, output_path, language=None, current_segment=None, total_segments=None, device=None):
...
def process_speaker_segments(vocals_path, diarization_data, output_dir, language=None, device=None):
    ...
    for speaker, segments in speaker_segments.items():
        ...
        for chunk_idx, chunk_segments in enumerate(chunks):
            ...
            try:
                ...
                display.update_log(f"🗣️ Transcribing {speaker}'s audio (chunk {chunk_idx + 1}/{len(chunks)})...")
                ...
                for trans in transcription:
                    ...
                    for offset in segment_offsets:
                        if trans_start >= offset["concat_start"] and trans_start < offset["concat_start"] + offset["duration"]:
                ...
                logging.error(f"Failed to process {speaker} chunk {chunk_idx}: {str(e)}")
                if os.path.exists(chunk_path):
                    os.remove(chunk_path)
                if os.path.exists(transcript_path):
                    os.remove(transcript_path)
                raise RuntimeError(f"Processing failed for {speaker} chunk {chunk_idx}: {str(e)}") from e
            finally:
                if os.path.exists(chunk_path):
                    os.remove(chunk_path)
                if os.path.exists(transcript_path):
    ...
    try:
        if start_step <= 1:
            ...
            if not os.path.exists(vocals_path):
                raise FileNotFoundError(f"Cannot skip to step {start_step}: {vocals_path} not found")
        ...
        if start_step <= 2:
            ...
            if not os.path.exists(diarization_path):
                raise FileNotFoundError(f"Cannot skip to step {start_step}: {diarization_path} not found")
        
        with open(diarization_path) as f:
            diarization_data = json.load(f)
        
        output_dir = "output/speakers"
        transcriptions = process_speaker_segments(vocals_path, diarization_data, output_dir, language, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-speakers", "-n", type=int, help="Number of speakers (optional)")
    parser.add_argument("--language", "-l", help="Language code for transcription (e.g. 'en', 'es', 'fr')")
    parser.add_argument("--start-step", "-s", type=int, choices=[1,2,3], default=1, 
    ...
    main(args.input_audio, args.num_speakers, args.language, args.start_step, args.device) 