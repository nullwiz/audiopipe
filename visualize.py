...
def plot_waveform(audio_path: str, output_path: str | None = None) -> str:
    """Plot audio waveform.
...
def plot_diarization(
    ...
    with open(diarized_json_path) as f:
    ...
    fig, axs = plt.subplots(
        figsize=(12, 6), nrows=2, gridspec_kw={"height_ratios": [1, 3]}
    )
...
def plot_transcript(
    ...
    speakers = list({segment.get("speaker", "UNKNOWN") for segment in segments})
    ...
    if not consolidated and len(segments) > 100:
        print(
            "WARNING: This appears to be a non-consolidated transcript with many segments."
        )
        print(
            "For better visualization with less Y-axis overflow, use the consolidated transcript:"
        )
        print("1. Run: python process_transcript.py")
        print(
            "2. Visualize: python visualize.py transcript output/final_transcription_consolidated.json"
        )
    ...
    if consolidated:
        ...
        fig_height = min(
            40, max(8, len(segments) * 0.15)
        )  # Much more compact height per segment
        # If we have too many segments, cap the height and warn
        if len(segments) > 300:
            fig_height = 40  # Maximum reasonable height
            print(
                f"WARNING: Large number of segments ({len(segments)}). Capping visualization height."
            )
            print(
                "Consider using the consolidated transcript for better visualization."
            )
    ...
    segment_spacing = (
        0.8 if consolidated else 0.4
    )  # Much tighter spacing for raw transcripts
    ...
    for _i, segment in enumerate(segments):
        ...
        if highlight_keywords and any(
            keyword.lower() in text.lower() for keyword in highlight_keywords
        ):
            ax.text(
                start + 0.1,
                y_pos - 0.5,
                ...
                fontsize=9,
                wrap=True,
                bbox={"facecolor": "lightyellow", "alpha": 0.5, "pad": 2},
            )
...
def generate_html_report(
    transcript_path: str,
    audio_path: str | None = None,
    diarized_json_path: str | None = None,
    output_path: str | None = None,
) -> str | None:
    ...
    total_duration = max(segment["end"] for segment in segments) - min(
    ...
    js_script = js_script.replace(
        "TIMELINE_DATA_PLACEHOLDER", json.dumps(timeline_data)
    )
...
def main():
    ...
    diar_parser.add_argument(
        "--audio", "-a", help="Path to audio file for waveform background"
    )
    diar_parser.add_argument("--output", "-o", help="Output path for plot")

    # Transcript command
    transcript_parser = subparsers.add_parser(
        "transcript", help="Plot transcript timeline"
    )
    ...
    transcript_parser.add_argument(
        "--highlight", "-hl", nargs="+", help="Keywords to highlight in transcript"
    )
    ...
    report_parser.add_argument(
        "--diarization", "-d", help="Path to diarization JSON file"
    )
    ...
    if args.command == "waveform":
        ...
        if not args.consolidated and "consolidated" not in args.json_path:
            ...
            if os.path.exists(consolidated_path):
                print(
                    f"TIP: A consolidated transcript was found at: {consolidated_path}"
                )
                print(
                    "     For better visualization with less overflow, consider using it instead."
                )
                print(
                    f"     Command: python visualize.py transcript {consolidated_path}"
                )
                print("     Continuing with requested file...")

        output_path = plot_transcript(
            args.json_path, args.output, args.consolidated, args.highlight
        )
        ...
        if "consolidated" not in args.transcript_path:
            consolidated_path = args.transcript_path.replace(
                ".json", "_consolidated.json"
            )
            if os.path.exists(consolidated_path):
                print(
                    f"TIP: A consolidated transcript was found at: {consolidated_path}"
                ...
                print(
                    "     For better report visualization, consider using it instead."
                )
                print(
                    f"     Command: python visualize.py report {consolidated_path}"
                    + (f" --audio {args.audio}" if args.audio else "")
                )
                print("     Continuing with requested file...")

        output_path = generate_html_report(
            args.transcript_path, args.audio, args.diarization, args.output
        )
        print(f"HTML report saved to: {output_path}")
