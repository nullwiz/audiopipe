...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--hf-token", action="store", 
        help="Hugging Face token for API access during tests"
    )

def pytest_configure(config):
    """Register markers for our tests"""
    config.addinivalue_line("markers", "integration: mark test as requiring integration with external systems")
    config.addinivalue_line("markers", "slow: mark test as slow")
    config.addinivalue_line("markers", "requires_token: mark test as requiring Hugging Face token")
    
    # Set Hugging Face token if provided
    hf_token = config.getoption("--hf-token")
    if hf_token:
        os.environ["HUGGING_FACE_TOKEN"] = hf_token
        print(f"Using provided Hugging Face token for tests")
    elif os.environ.get("HUGGING_FACE_TOKEN"):
        print(f"Using Hugging Face token from environment for tests")
    else:
        warnings.warn(
